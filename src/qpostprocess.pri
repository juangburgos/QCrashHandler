...
linux-g++ {
  ...
  QMAKE_POST_LINK += $$quote( cd "$${DESTDIR}"; objcopy --only-keep-debug "$${TARGET_FULL}" "$${TARGET_PDB}" $$escape_expand(\n\t))
  QMAKE_POST_LINK += $$quote( cd "$${DESTDIR}"; strip --strip-debug --strip-unneeded "$${TARGET_FULL}" $$escape_expand(\n\t))
  QMAKE_POST_LINK += $$quote( cd "$${DESTDIR}"; objcopy --add-gnu-debuglink="$${TARGET_PDB}" "$${TARGET_FULL}" $$escape_expand(\n\t))
}